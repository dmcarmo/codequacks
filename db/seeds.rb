...
puts "Creating ducks..."
java_script = Duck.create(name: "Jay Scripped", description: "Useful javascript functions and classes. The main helper is fw , an object containing constants, methods and classes.", price: 23.00, user: sylvia)
...
database = Duck.create(name: "Greatest Database Duck, just really great", description: "What I've seen, what I've seen is so bad, because I am the greatest, just really great, we'll got, like you wouldn't believe data, so much data, all the data, database duck you'll ever find, other so call The Donalds that are ducks are just losers! The concept of global warming was created by and for the Chinese in order to make U.S. manufacturing non-competitive.", price: 10.00, user: isaline)
...
css = Duck.create(name: "CSSuperwoman", description: "Relax citizen! I will fix all your CSS troubles faster than a speeding bullet!", price: 15.00, user: sylvia)
...
html = Duck.create(name: "Dr. H.T Emell", description: "Well organized and easy to understand Web building tutorials with lots of examples of how to use HTML", price: 15.00, user: david_carmo)
...
rails = Duck.create(name: "Dr. Ruabbi O'Railly", description: "The Rails framework provides a large number of helpers for working with assets, dates, forms, numbers and model objects, to name a few.", price: 20.00, user: david_wylie)
...
php = Duck.create(name: "Ms Peppermint H Potter", description: "Please take your seat, you are already far behind in your coding study to be lollygagging around!.", price: 30.00, user: david_wylie)
...
python = Duck.create(name: "Pie? aye son!", description: "O ye'll tak' the high road, and I'll tak' the low road, And I'll be in Scodeland a'fore ye...", price: 30.00, user: david_wylie)
file = URI.open('https://www.nationalgalleries.org/sites/default/files/product_images/bagpiper3.jpg')
ruby.photo.attach(io: file, filename: 'nes6.png', content_type: 'image/png')

node = Duck.create(name: "Fire Duckpartment", description: "If your keyboard bursts into flames from frantic coding, I'll deal with it!.", price: 30.00, user: david_wylie)
...
java = Duck.create(name: "Monsieur Ja'Va", description: "This is a small Ruby tutorial that should take no more than 20 minutes to complete. It makes the assumption that you already have Ruby installed.", price: 30.00, user: david_wylie)
